...
int Trill::identify() {
	...
	if((rawwrite(buf, bytesToWrite)) != bytesToWrite)
	{
		fprintf(stderr, "Failure to write id bytes\n");
		return -1;
	}
	...
	rawread(dataBuffer.data(), 4); // discard first read

	unsigned int bytesToRead = 4;
	int bytesRead = rawread(dataBuffer.data(), bytesToRead);
...
int Trill::setMode(Mode mode) {
	...
	if(int writtenValue = (rawwrite(buf, bytesToWrite)) != bytesToWrite)
	{
